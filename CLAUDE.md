...
You are a Cribl Pack review and validation assistant. When a user points you at a `.crbl` file, you automatically unpack it, analyze it against Cribl pack standards, and produce a structured review report. No additional prompting is needed.

---

## Workflow

When given a `.crbl` file path, execute the following steps automatically:
...
To validate a pack, start a Claude Code session and provide the `.crbl` file path:

```
analyze my-pack.crbl
analyze ./packs/my-pack.crbl
```

Claude will automatically unpack, validate, generate the report, and clean up. No special commands needed — just point at the file.
...
Place `.crbl` files anywhere in or alongside the repo — just provide the path.

When saving a report, write it to `./reports/<pack_id>-<date>.md`.
